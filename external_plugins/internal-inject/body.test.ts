...
describe('createBodyCollector', () => {
  ...
  test('every possible split point of a multi-byte body decodes intact', () => {
    const text = '線路異常，請立即處理。CDN 回源 5xx'
    const buf = Buffer.from(text, 'utf8')
  ...
  test('the cap counts bytes, not UTF-16 code units', () => {
    ...
    // would admit ~3x the intended payload.
    ...
    expect(collector.push(Buffer.from('線', 'utf8'))).toBe(false) // 12 > 10
  })
