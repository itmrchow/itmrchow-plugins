...
const KEY = 'INTERNAL_INJECT_PORT'
...
describe('resolveInjectPort', () => {
  ...
  test('honours a valid override', () => {
    expect(resolveInjectPort('7999', DEFAULT_PORT, KEY)).toBe(7999)
  })
  ...
  test('falls back on a trailing-garbage value rather than silently truncating it', () => {
    expect(resolveInjectPort('7999abc', DEFAULT_PORT, KEY)).toBe(DEFAULT_PORT)
  })
