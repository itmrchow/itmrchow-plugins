...
const mcp = new Server(
  { name: 'telegram', version: '1.0.0' },
  {
    ...
    instructions: [
      ...
      '',
      'MANDATORY — a <channel source="telegram"> tag means a person is waiting in Telegram. Work is not delivered until it has gone out through the reply tool with the chat_id from that tag; transcript output does not count, however complete it looks.',
      '- Every output meant for a person — a question you need answered, an interim conclusion, the final result — goes out through reply.',
      '- After each externally visible action (creating or updating a ticket, opening a merge/pull request, running a command that changes something outside this session, dispatching a subagent), send one short reply saying what you just did.',
      '- Never end a turn that started from such a tag without at least one reply. On a long task do not save it all for the end: report as you go, then send the final result as a new reply.',
      '- If you are blocked, or you decide no action is needed, say so through reply as well — from the sender\'s side, silence is indistinguishable from a crash.',
      '',
      'Messages from Telegram arrive as <channel source="telegram" chat_id="..." message_id="..." user="..." ts="...">. If the tag has an image_path attribute, Read that file — it is a photo the sender attached. If the tag has attachment_file_id, call download_attachment with that file_id to fetch the file, then Read the returned path. If the sender quote-replied to an earlier message, the tag carries reply_to_message_id (the referenced message\'s ID), reply_to_user (its author, "me" = this bot), and reply_to_text (the referenced message\'s full body, already inlined by Telegram — no extra fetch, and none is possible since Telegram exposes no history API). Read reply_to_text directly to resolve what "this"/"that" refers to. (Field names match the Discord channel; there reply_to_text is absent and you must call get_message — here Telegram hands you the full text up front.) Use reply_to (set to a message_id) only when replying to an earlier message; the latest message doesn\'t need a quote-reply, omit reply_to for normal responses.',
      '',
...
async function deliverToChannel({ content, meta }: ChannelDelivery): Promise<void> {
  try {
    ...
    process.stderr.write(
      `telegram channel: delivered (scope=${AGENT_SCOPE} chat_id=${meta.chat_id} message_id=${meta.message_id ?? '-'})\n`,
    )
  } catch (err) {
