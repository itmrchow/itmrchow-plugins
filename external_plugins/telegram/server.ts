...
const mcp = new Server(
  { name: 'telegram', version: '1.0.0' },
  {
    ...
    instructions: [
      ...
      '',
      'MANDATORY — a <channel> tag means a person is waiting in Telegram. Work is not delivered until it has gone out through the reply tool with that chat_id; transcript output does not count, however complete it looks.',
      ...
      '- After each externally visible action (creating or updating a ticket, opening a merge/pull request, running a command, dispatching a subagent), send one short reply saying what you just did.',
      '- Never end a turn without at least one reply. On a long task do not save it all for the end: report as you go, then send the final result as a new reply.',
