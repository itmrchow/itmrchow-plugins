...
describe('applyBind', () => {
  test('records a first-time redeemer', () => {
    ...
    expect(applyBind(invite, 'telegram', '111')).toBe(true)
    expect(invite.usedBy).toEqual({ telegram: ['111'] })
  })

  test('is a no-op when the same sender redeems again', () => {
    const invite = makeInvite({ usedBy: { telegram: ['111'] } })
    expect(applyBind(invite, 'telegram', '111')).toBe(false)
    expect(invite.usedBy.telegram).toEqual(['111'])
  })

  test('appends a second distinct sender on the same platform', () => {
    const invite = makeInvite({ usedBy: { telegram: ['111'] } })
    expect(applyBind(invite, 'telegram', '222')).toBe(true)
    expect(invite.usedBy.telegram).toEqual(['111', '222'])
  })

  test('creates a new platform bucket without touching existing ones', () => {
    const invite = makeInvite({ usedBy: { telegram: ['111'] } })
    expect(applyBind(invite, 'discord', '999')).toBe(true)
    expect(invite.usedBy).toEqual({ telegram: ['111'], discord: ['999'] })
  })
})
