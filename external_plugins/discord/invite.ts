// Invite tokens: an admin hands out a token out-of-band (plain text — Discord
// has no deep-link mechanism), the holder redeems it via /start <token> and
// lands in allowFrom. Opposite direction from pairing (bot mints a code, human
// approves after the fact), so `pending` is not reused — the two coexist.
//
// Deliberately platform-agnostic and I/O-free: no discord.js, no fs. Reading
// and writing access.json stays in server.ts so these stay unit-testable. This
// file is a copy of telegram/invite.ts, matching the per-platform duplication
// this directory already uses (meta-text.ts / inject-port.ts).
 ...
 * @param platform Channel name, e.g. 'discord'.
